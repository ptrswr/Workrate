...
export class EventService {
  ...
  protected convertDateFromClient(event: IEvent): IEvent {
    return Object.assign({}, event, {
      start_date: event.start_date?.isValid() ? event.start_date.toJSON() : undefined,
      end_date: event.end_date?.isValid() ? event.end_date.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((event: IEvent) => {
        event.start_date = event.start_date ? dayjs(event.start_date) : undefined;
        event.end_date = event.end_date ? dayjs(event.end_date) : undefined;
      });
    }
    return res;
  }
